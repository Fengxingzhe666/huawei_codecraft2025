...
横坐标：表示第i个1800时间片，例如1表示时间片1~1800，2表示时间片1801~3600，以此类推。纵坐标：表示在1800时间片内某tag被读请求的次数。  
